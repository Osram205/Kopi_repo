...
class CalificacionService:
    ...
    def crear(db: Session, request: calificacion_schema.CalificacionCrear, usuario: models.Usuario):
        if not 1 <= request.puntuacion <= 5: raise HTTPException(status_code=422, detail="La puntuación debe estar entre 1 y 5.")
        if request.evaluado_id == usuario.id: raise HTTPException(status_code=422, detail="No puedes calificarte a ti mismo.")
        ...
        if not es_conductor and not es_pasajero: raise HTTPException(status_code=403, detail="Solo participantes del viaje pueden calificar.")
