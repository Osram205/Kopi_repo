...
def crear_pago(
    request: pago_schema.PagoCrear,
    db: Session = Depends(database.get_db),
    usuario: models.Usuario = Depends(get_current_user),
):
    if request.metodo_pago not in {"tarjeta", "transferencia"}:
    ...
    if reservacion.pasajero_id != usuario.id:
        raise HTTPException(status_code=403, detail="Solo el pasajero puede registrar el pago.")
    if reservacion.estatus_reserva != models.EstatusReserva.aceptado:
        raise HTTPException(status_code=422, detail="La reservación debe estar aceptada para registrar pago.")

    existente = db.query(models.Pago).filter(models.Pago.reservacion_id == reservacion.id).first()
    if existente:
        raise HTTPException(status_code=422, detail="La reservación ya tiene un pago registrado.")
...
def actualizar_estatus_pago(
    pago_id: int,
    request: pago_schema.PagoEstatus,
    db: Session = Depends(database.get_db),
    usuario: models.Usuario = Depends(get_current_user),
):
    if request.estatus_pago not in {"pendiente", "completado", "reembolsado"}:
        raise HTTPException(status_code=422, detail="Estatus de pago inválido.")
    ...
    if request.estatus_pago == "completado":
